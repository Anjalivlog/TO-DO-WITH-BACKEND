...
const todoDataValidation = ({ todo }) => {
    return new Promise((resolve, reject) => {
        ...
        if (todo.length < 3 || todo.length > 100) reject('Todo length should be 3-100 chars only.');
...
const sendVerificationMail = (email, token) => {
    ...
    const transpoter = nodemailer.createTransport({
        ...
        service: "gmail",
        auth: {
            user: "noeenqueen@gmail.com",
            pass: 'hkic byxg kanw xqec '
        }
    });

    //mail options
    const mailOptions = {
        from: 'noeenqueen@gmail.com',
        to: email,
